...
class InvertedIndex:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for doc_id, tokens in enumerate(document_tokens, start=start_doc_id):
                ...
                cursor.execute("INSERT INTO document (id, url, summary) VALUES (?, ?, ?)",
                               (doc_id, f"http://example.com/{doc_id}", ""))
    ...
    def search(self, query_tokens: List[str], num_results: int):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for term in query_tokens:
                ...
                cursor.execute("SELECT DISTINCT document_id FROM posting WHERE term_id = ?", (term_id,))
    ...
    def _get_avg_doc_length(self) -> float:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT AVG(length) FROM (SELECT COUNT(*) AS length FROM posting GROUP BY document_id)")
